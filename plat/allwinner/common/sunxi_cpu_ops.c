...
static void sunxi_cpu_off(u_register_t mpidr)
{
	...
	NOTICE("wb8: cpu_off c%u clamp=0x%x\n", core,
	       mmio_read_32(SUNXI_CPU_POWER_CLAMP_REG(cluster, core)));
}
...
void sunxi_cpu_on(u_register_t mpidr)
{
	...
	/*
	 * wb8 console instrumentation: this runs at EL3 during the
	 * kernel-triggered CPU_ON (console is RUNTIME-scoped), so it prints
	 * live over the shared UART. Shows entry, the branch taken, and the
	 * retained POWER_CLAMP (0x00 would risk enable_power's early-return).
	 */
	NOTICE("wb8: cpu_on c%u mpidr=0x%lx percluster=%u clamp=0x%x\n",
	       core, (unsigned long)mpidr,
	       (unsigned int)sunxi_cpucfg_has_per_cluster_regs(),
	       mmio_read_32(SUNXI_CPU_POWER_CLAMP_REG(cluster, core)));
	...
	if (sunxi_cpucfg_has_per_cluster_regs()) {
		...
		mmio_write_32(SUNXI_ALT_RVBAR_HI_REG(core),
			      sunxi_sec_entrypoint >> 32);
		NOTICE("wb8: cpu_on c%u else-path rearm ALTrvbar=0x%x clamp=0x%x\n",
		       core, mmio_read_32(SUNXI_ALT_RVBAR_LO_REG(core)),
		       mmio_read_32(SUNXI_CPU_POWER_CLAMP_REG(cluster, core)));
		/*
		 * wb8 instrumentation (RTC GP7 @ 0x0700011c): non-per-cluster
		 * CPU_ON path, so the branch and re-armed vector are visible:
		 *  [31:28] core  [27:24] 0xD sig (0xD=else path, 0xC=per-cluster)
		 *  [23:16] POWER_CLAMP readback after ramp (00=on, ff=off)
		 *  [15:0]  ALT_RVBAR_LO(core) low 16 bits (non-zero => re-armed)
		 */
		if (core != 0U) {
			mmio_write_32(0x0700011cU,
				(((uint32_t)core & 0xfU) << 28) | (0xDU << 24) |
				((mmio_read_32(SUNXI_CPU_POWER_CLAMP_REG(cluster, core)) & 0xffU) << 16) |
				(mmio_read_32(SUNXI_ALT_RVBAR_LO_REG(core)) & 0xffffU));
		}
#endif
		/* ??? Deassert CPU power-on reset ??? */
